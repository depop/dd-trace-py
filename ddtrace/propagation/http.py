...
from ..settings import _config as _dd_config
...
class HTTPPropagator(BaseHTTPPropagator):
    ...
    def inject(span_context, headers):
        ...
        """
        if "datadog" in _dd_config.propagation_style_inject:
            DatadogHTTPPropagator.inject(span_context, headers)

        if "b3" in _dd_config.propagation_style_inject:
    ...
    def extract(headers):
        ...
        if "datadog" in _dd_config.propagation_style_extract:
            ctx = DatadogHTTPPropagator.extract(headers)
            if ctx != empty_ctx:
                return ctx

        if "b3" in _dd_config.propagation_style_extract:
