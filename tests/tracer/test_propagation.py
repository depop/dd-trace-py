...
class TestHttpPropagation(BaseTestCase):
    ...
    def test_extract_datadog(self):
        with self.override_env(dict(DD_PROPAGATION_STYLE_EXTRACT="Datadog")):
            tracer = DummyTracer()
...
class TestDatadogHttpPropagation(BaseTestCase):
...
class TestB3HttpPropagation(BaseTestCase):
    def test_inject(self):
        tracer = DummyTracer()
